...
const RoomPage: React.FC = () => {
  ...
  const [vidStreams, setvidStreams] = useState<{
    [key: string]: React.MutableRefObject<HTMLVideoElement>;
  }>({ me: localVideoRef });
  ...
  useEffect(() => {
    ...
    const connectToNewUser = (
      ...
      const remoteVideoRef =
        React.createRef() as React.MutableRefObject<HTMLVideoElement>;
      ...
      call.on("close", () => {
        ...
        setvidStreams(
          (oldVidStream) =>
            Object.fromEntries(
              Object.entries(oldVidStream).filter(
                ([key, value]) => key !== call.peer
              )
            ) as { [key: string]: React.MutableRefObject<HTMLVideoElement> }
        );
      });
    };
    console.log("adding your cam", socket, peer);
    navigator.mediaDevices.getUserMedia(userPrefs).then((stream) => {
  ...
  }, [peer, roomId, socket, peers, userPrefs]);
  // useEffect(() => {
  ...
  return (
    <div className="bg-Cultured 2 w-3/5 h-4/5">
      ...
      <VControllers
        isMute={!userPrefs.audio}
        isVideoOpen={userPrefs.video}
        toggleMic={toggleMic}
